// Seed resource data — first-run defaults imported into the backend when no
// legacy localStorage source exists (see hooks/useRemoteSeed.js). The backend is
// the application-data source of truth; localStorage holds UI prefs only.
// Shape: docs/DATA_MODEL.md → Resource.

export const RESOURCES = [
  {
    id: 'e1f3a5b7-9c24-4d86-a0e2-3f4c5d6e7f80',
    title: 'React official docs — Thinking in React',
    url: 'https://react.dev/learn/thinking-in-react',
    category: 'article',
    description: 'Guide to structuring React apps from scratch.',
    pinned: true,
    createdAt: '2026-08-01T12:00:00.000Z',
  },
  {
    id: 'f2a4b6c8-0d35-4e97-b1f3-4a5b6c7d8e91',
    title: 'Tailwind CSS documentation',
    url: 'https://tailwindcss.com/docs',
    category: 'tool',
    description: 'Utility-first CSS framework reference.',
    pinned: true,
    createdAt: '2026-08-05T08:30:00.000Z',
  },
  {
    id: 'a3b5c7d9-1e46-4f08-c2a4-5b6c7d8e9f02',
    title: 'JavaScript.info — The Modern Tutorial',
    url: 'https://javascript.info',
    category: 'course',
    description: 'Comprehensive modern JavaScript tutorial.',
    pinned: false,
    createdAt: '2026-08-10T14:15:00.000Z',
  },
  {
    id: 'b4c6d8e0-2f57-4a19-d3b5-6c7d8e9f0a13',
    title: 'Accessible color palette generator',
    url: 'https://coolors.co/palettes/trending',
    category: 'tool',
    description: 'Browse and create accessible color palettes.',
    pinned: false,
    createdAt: '2026-08-12T11:40:00.000Z',
  },
  {
    id: 'c5d7e9f1-3a68-4b20-e4c6-7d8e9f0a1b24',
    title: 'A11y Project — Web Accessibility Checklist',
    url: 'https://www.a11yproject.com/checklist/',
    category: 'article',
    description: 'Practical accessibility checklist for web projects.',
    pinned: false,
    createdAt: '2026-08-15T09:20:00.000Z',
  },
]